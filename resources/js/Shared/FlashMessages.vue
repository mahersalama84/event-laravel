<template>
    <div>
        ...
        >
            <div class="flex items-center">
                ...
                <div class="py-4 text-white text-sm font-medium">
                    {{
                        $page.props.flash &&
                        __("flashMessages")[$page.props.flash.success]
                    }}
                </div>
            </div>
            <button type="button" class="group mx-2 p-2" @click="show = false">
                <svg
        ...
        >
            <div class="flex items-center">
                <svg
                    class="flex-shrink-0 ml-4 mx-2 w-4 h-4 fill-white"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20"
                >
                    <path
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    <span v-if="Object.keys($page.props.errors).length === 1">{{
                        __("flashMessages")["one_error"]
                    }}</span>
                    <span v-else>
                        {{ __("flashMessages")["there_are"] }}
                        {{ Object.keys($page.props.errors).length }}
                        {{ __("flashMessages")["form_errors"] }}
                    </span>
                    <!-- <span v-else
                        >There are
